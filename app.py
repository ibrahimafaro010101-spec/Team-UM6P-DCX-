...
# Ajouter après les autres imports existants
...
st.caption("Transformer vos données en décisions viables")
...
with st.sidebar:
    ...
    # Pré-remplir depuis .env si disponible
    api_key_input = st.text_input(
        "Clé OpenAI API",
        type="password",
        value=OPENAI_API_KEY,  # Pré-remplir depuis .env
...
# ============================================================
# ============================================================
# PAGE 2: TRAITEMENT DES DONNÉES - VERSION CORRIGÉE
# ============================================================
elif page == " Traitement des données":
    st.header("🔧 Traitement Scientifique des Données")
    ...
    try:
        ...
        if 'data_processor' not in st.session_state or st.session_state.data_processor is None:
            ...
            st.success("✅ Moteur de traitement scientifique initialisé")
    ...
    # ============================================================
    with tab2:
        st.subheader("🔧 Prétraitement Scientifique")

        st.markdown("""
        **🛠️ Options de prétraitement:**
...
# ============================================================
# 4️⃣ NLQ SÉCURISÉ - VERSION COMPLÈTE
# ============================================================
elif page == " NLQ Sécurisé":
    st.header("🔒 NLQ (Natural Language Query) Sécurisé")

    if not st.session_state.data_loaded:
        st.warning("⚠️ Veuillez d'abord charger des données dans l'onglet 'Chargement des données'")
        st.stop()

    # Vérification et initialisation du moteur NLQ
    if st.session_state.nlq_engine is None:
        try:
            ...
            if hasattr(st.session_state, 'openai_client') and st.session_state.openai_client:
                ...
                # Essayer de récupérer la clé depuis l'input
            ...
            # Initialiser le moteur NLQ
            st.session_state.nlq_engine = SecureNLQEngine(api_key=api_key)
            st.success("✅ Moteur NLQ sécurisé initialisé")

        except ImportError as e:
            st.error(f"❌ Module secure_nlq_engine non disponible : {e}")
            st.info("""
            **Assurez-vous que :**
            1. Le fichier `secure_nlq_engine.py` est dans le dossier `modules/`
            2. Les dépendances sont installées : `pip install openai pandas`
            """)
            st.stop()
        except Exception as e:
    ...
    if st.session_state.nlq_engine is None:
        st.error("❌ Le moteur NLQ n'a pas pu être initialisé")
        st.stop()
    ...
    # Initialiser l'historique NLQ
    if 'nlq_history' not in st.session_state:
        st.session_state.nlq_history = []

    # Initialiser une variable pour la requête en cours
    if 'current_nlq_query' not in st.session_state:
        st.session_state.current_nlq_query = ""

    # Onglets pour différents modes d'analyse
    tab1, tab2, tab3, tab4 = st.tabs([
        "📝 Analyse par requête",
        "📊 Métadonnées",
        "🔄 Historique",
    ...
    with tab1:
        ...
        # Description avec informations importantes
        st.info("""
        **🎯 FONCTIONNALITÉ AVANCÉE :**  
        Posez des questions en langage naturel sur vos données. Le système génère des analyses, 
        des visualisations et des scripts sans exposer vos données brutes.

        **🔒 SÉCURITÉ MAXIMALE :**  
        L'analyse se fait uniquement sur les **métadonnées** (noms et types de colonnes).  
        Vos données sensibles **ne quittent jamais** votre environnement.

        **📊 PRÉCISION ASSURANCE :**  
        Recommandations spécialisées pour le domaine de l'assurance automobile.
        """)

        # Section de requête
        col_query, col_guidance = st.columns([3, 1])

        with col_query:
            # Zone de requête avec placeholder amélioré
            user_query = st.text_area(
                "**💬 Posez votre question d'analyse :**",
                height=120,
                placeholder="""Exemples :
• Quels sont les facteurs influençant les primes d'assurance ?
• Quelle est la distribution des sinistres par type de véhicule ?
• Comment segmenter les clients par niveau de risque ?
• Quelles variables prédisent le mieux les sinistres coûteux ?
• Comment optimiser les tarifs par région géographique ?""",
                help="Formulez votre question en français naturel. L'IA analysera la structure de vos données et proposera une méthodologie.",
                key="nlq_query_text_area"
            )
        ...
        with col_guidance:
            ...
            for tip in formulation_tips:
                st.markdown(f"• {tip}")

        # Bouton d'analyse
        if st.button("🔬 Lancer l'analyse NLQ", type="primary", key="nlq_analyze_btn"):
            if user_query.strip():
                with st.spinner("🧠 Analyse NLQ en cours..."):
                    try:
                        ...
                        if st.session_state.metadata:
                            ...
                            for col in df.columns:
                                col_info = {
                                    ...
                                    'exemple_valeurs': str(df[col].iloc[0]) if len(df) > 0 else ''
                                }
                            ...
                            metadata = {
                                ...
                                'general_info': {
                                    ...
                                    'format_source': 'DataFrame Python'
                                },
                                'business_context_hints': {
                                    'domaine': 'assurance',
                                    'themes_identifies': ['assurance_auto'],
                                    'variables_cles': [col for col in df.columns if any(
                                        kw in col.lower() for kw in
                                        ['prime', 'sinistre', 'risque', 'client', 'vehicule']
                                    )]
                                }
                            }
                        ...
                        if "error" in result:
                            ...
                            st.success("✅ Analyse NLQ terminée avec succès !")
                            ...
                            intention = analysis_data.get("intention", "Analyse générée automatiquement")
                            st.info(f"**🎯 Intention détectée :** {intention}")

                            # Réponse détaillée
                            detailed_response = analysis_data.get("reponse_detaillee", "")
                            if detailed_response:
                                st.markdown("#### 📝 Réponse complète")
                                st.markdown(detailed_response)

                            # Stratégie
                            st.subheader("🎯 Stratégie d'analyse")
                            strategie = analysis_data.get("strategie_analyse", "")
                            if strategie:
                                st.markdown(strategie)

                            # Visualisations recommandées
                            st.subheader("📊 Visualisations recommandées")
                            visualisations = analysis_data.get("visualisations_suggestees", [])
                            if visualisations:
                                for i, viz in enumerate(visualisations, 1):
                                    st.markdown(
                                        f"{i}. **{viz.get('type', 'Graphique')}** - {viz.get('description', '')}")

                    except Exception as e:
                        st.error(f"❌ Erreur lors de l'analyse NLQ : {str(e)}")
                        st.code(traceback.format_exc())
            else:
    ...
    # ============================================
    with tab2:
        st.subheader("📊 Métadonnées disponibles")
        ...
        if st.session_state.metadata:
            ...
            if columns_info:
                ...
                st.dataframe(columns_df[['nom', 'type_donnee', 'description']].head(10),
                             use_container_width=True)
        else:
            st.info(
                "⚠️ Aucune métadonnée disponible. Veuillez d'abord extraire les métadonnées dans l'onglet correspondant.")
    ...
    with tab3:
        ...
        if st.session_state.nlq_history:
            for i, item in enumerate(reversed(st.session_state.nlq_history)):
                with st.expander(f"📅 {item.get('timestamp', 'Sans date')} - {item['query'][:50]}...",
                                 expanded=i == 0):
                    st.markdown(f"**Question :** {item['query']}")

                    if 'result' in item and 'analysis' in item['result']:
                        analysis = item['result']['analysis']
                        st.markdown(f"**Intention :** {analysis.get('intention', 'N/A')}")

                        if 'reponse_detaillee' in analysis:
                            st.markdown("**Réponse :**")
                            st.markdown(analysis['reponse_detaillee'][:500] + "...")
                    elif 'error' in item:
                        st.error(f"Erreur : {item['error']}")
        else:
    ...
    with tab4:
        ...
        if st.session_state.nlq_engine:
            st.success("✅ Moteur NLQ initialisé et opérationnel")

            # Informations sur la sécurité
            st.markdown("#### 🔒 Niveau de sécurité")
            st.info("""
            **Mode sécurisé activé :**
            - Analyse uniquement sur métadonnées
            ...
            include_visualizations = st.checkbox("Inclure les suggestions de visualisation", value=True)
            include_code = st.checkbox("Inclure le code d'implémentation", value=True)

            if st.button("💾 Sauvegarder les paramètres", type="secondary"):
                st.success("Paramètres sauvegardés")
        else:
            st.error("❌ Moteur NLQ non initialisé")
#=# ============================================================
# 5️⃣ INSIGHTS AVANCÉS - SECTION COMPLÈTEMENT CORRIGÉE
# ============================================================
elif page == " Insights Avancés":
    st.header("🎯 Insights Avancés et Analyse du Risque Client")

    # VÉRIFICATION : Vérifier si les données sont chargées
    if not st.session_state.data_loaded:
        st.warning("⚠️ Veuillez d'abord charger des données dans l'onglet 'Chargement des données'.")
        st.stop()

    # CHOIX : Utiliser df_final si disponible, sinon dataframe
    if st.session_state.df_final is not None:
        df_final = st.session_state.df_final
    else:
        df_final = st.session_state.dataframe

    # INITIALISATION : Initialiser insight_engine si nécessaire
    ...
    # RÉFÉRENCE : Utiliser l'engine de la session state
    insight_engine = st.session_state.insight_engine

    # CONSTRUCTION : Construire la table client et calculer les scores
    with st.spinner("🔍 Construction de la table client et calcul des risques..."):
        try:
            ...
            # Génération des insights individuels
            ...
            st.info("Les colonnes requises (ncli, nomncli, Prime, nb_jour_couv) doivent être présentes")
            st.stop()
        except Exception as e:
    ...
    # AFFICHAGE : Menu des types d'analyse
    st.markdown("---")
    ...
    if analysis_type == "📊 Vue d'ensemble":
        ...
        with col4:
            ...
            st.metric("⚠️ Risque élevé", f"{high_risk} clients")
        ...
        # Tableau des clients à haut risque
        ...
        available_columns = []
        if 'nomncli' in high_risk_clients.columns:
            available_columns.append('nomncli')
        if 'score_risque' in high_risk_clients.columns:
            available_columns.append('score_risque')
        if 'prime_totale' in high_risk_clients.columns:
            available_columns.append('prime_totale')
        if 'frequence_sinistre' in high_risk_clients.columns:
            available_columns.append('frequence_sinistre')
        if 'insight' in high_risk_clients.columns:
            available_columns.append('insight')

        # Afficher uniquement les colonnes disponibles
        if available_columns:
            st.dataframe(
                high_risk_clients[available_columns],
                use_container_width=True
            )
        else:
    ...
    # ============================================================
        ...
        fig = insight_engine.create_bivariate_scatter(
            ...
            x_var,
            y_var,
            color_var if color_var != 'Aucune' else None
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        try:
            ...
            with col_corr3:
                if correlation > 0:
                    st.markdown(f"**Interprétation :** Quand {x_var} augmente, {y_var} tend à augmenter")
                elif correlation < 0:
                    st.markdown(f"**Interprétation :** Quand {x_var} augmente, {y_var} tend à diminuer")
                else:
                    st.markdown("**Interprétation :** Pas de relation linéaire évidente")
        ...
        if multivariate_choice == "Analyse en Composantes Principales (ACP)":
            ...
            if st.button("🔬 Lancer l'ACP", type="primary"):
                with st.spinner("Calcul de l'ACP en cours..."):
                    try:
                        pca_result = insight_engine.perform_pca_analysis(
                            df=scored_clients,
                        ...
                        if 'error' in pca_result:
                            st.error(f"❌ Erreur ACP: {pca_result['error']}")
                        else:
                            st.success("✅ ACP terminée avec succès !")

                            # Affichage des résultats
                            pca_viz = insight_engine.create_pca_visualization(pca_result, scored_clients)

                            if pca_viz and 'variance_plot' in pca_viz:
                                st.plotly_chart(pca_viz['variance_plot'], use_container_width=True)

                            # Tableau des chargements
                            if 'loadings_df' in pca_result:
                                st.subheader("📋 Chargements des variables (Loadings)")
                                st.dataframe(
                                    pca_result['loadings_df'].style.background_gradient(cmap='RdBu', axis=0),
                                    use_container_width=True
                                )
            ...
            if len(analysis_cols) >= 2:
                ...
                clustering_features = st.multiselect(
                    ...
                    default=available_recommended[:min(3, len(available_recommended))]
                )
                ...
                if st.button("🔬 Lancer le Clustering", type="primary"):
                    with st.spinner("Clustering en cours..."):
                        try:
                            ...
                            cluster_summary = scored_clients_clustered.groupby('Cluster').agg({
                                'score_risque': 'mean',
                                'prime_totale': 'mean',
                                'frequence_sinistre': 'mean',
                                'niveau_risque': lambda x: x.mode()[0] if not x.mode().empty else 'N/A',
                                'nomncli': 'count'
                            }).round(2)
    ...
    # ============================================================
        ...
        with col2:
            ...
            csv_data = scored_clients[['nomncli', 'score_risque', 'niveau_risque', 'prime_totale',
                                      'frequence_sinistre', 'insight']].to_csv(index=False, encoding='utf-8-sig')
...
st.markdown(
    ...
    <p><b> LIK Insurance Analyst </b> | Transformer vos données en décisions viables</p>
    <p> Année 2026-2027</p>
    </div>
    """,
    unsafe_allow_html=True
